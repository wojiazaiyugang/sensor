...
from settings import MODEL_DIR, logger, np
...
class OneClassSvm:
    ...
    def _get_svm_format_data(self, data: List[np.ndarray]) -> List[dict]:
        """
    ...
    def predict(self, data: np.ndarray) -> list:
        ...
        :return:
        """
        assert isinstance(data, np.ndarray) and len(data.shape) == 3
        features = [self._get_feature(data_i) for data_i in data]
